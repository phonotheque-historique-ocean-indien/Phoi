...
La Réunion est envoûtante, multi-ethnique, authentique, sa culture se conjugue au pluriel avec la découverte de ses cases créoles colorées, ses jardins aux mille essences, ses temples hindous, ses églises, ses pagodes chinoises, ses mosquées, ses demeures coloniales, ses monuments historiques, son architecture créole, ses musées, ses festivals de musique et ses fêtes traditionnelles…";
                ...
                break;
            case "Seychelles":
                $description = "Les Seychelles forment un archipel de 115 îles (dont une artificielle), situé dans l'ouest de l'océan Indien et rattaché au continent africain. 
                Toutes ces îles sont regroupées en un État dont la capitale est la ville de Victoria sur l'île principale de Mahé.";
                $facet_id = 1016;
                break;
            case "Maurice":
                $description = "Maurice (en anglais : Mauritius), en forme longue la république de Maurice (en anglais : Republic of Mauritius), est un État insulaire de l'océan Indien à 868 kilomètres à l'est de Madagascar et 172 kilomètres à l'est-nord-est de La Réunion. 
                Le pays inclut l'île principale de Maurice, mais aussi l'île Rodrigues à 560 kilomètres à l'est de l'île principale. Les îles plus lointaines d'Agaléga et de Saint-Brandon font partie du territoire national. Les îles Maurice et Rodrigues font partie de l'archipel des Mascareignes, avec l'île de La Réunion qui est elle un département d'outre-mer français. La capitale et plus grande ville est Port-Louis.";
                $facet_id = 1064;
                break;
            case "Rodrigues":
        ...
        $o_results = $o_search->search('ca_objects.grands_types:"collectages" AND ca_objects.deleted:0 AND ca_objects.pays_liste:"'.$pays.'"');
        $nb_objects = $o_results->numHits();
        $this->view->setVar("nb_collectages", $nb_objects);

        $o_search = new ObjectSearch();
        $o_results = $o_search->search('ca_objects.grands_types:"autres" AND ca_objects.deleted:0 AND ca_objects.pays_liste:"'.$pays.'"');
        $nb_autres = $o_results->numHits();
        $this->view->setVar("nb_autres", $nb_autres);

        $o_browse = new ObjectSearch();
        $o_results = $o_search->search('ca_objects.grands_types:"oeuvres" AND ca_objects.deleted:0 AND ca_objects.pays_liste:"'.$pays.'"');
        $nb_oeuvres = $o_results->numHits();
        $this->view->setVar("nb_oeuvres", $nb_oeuvres);

        $o_browse = new ObjectSearch();
        $request = 'ca_objects.grands_types:"phonogrammes" AND ca_objects.pays_facet:"'.$pays.'"';
        $request = 'ca_objects.type_id:"878" AND ca_objects.deleted:0 AND ca_objects.pays_liste:"'.$pays.'"';
        $o_results = $o_search->search($request);
